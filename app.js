import {weeklyMetrics,releaseMetrics,trendMetrics,blankDraft,emptyWeek,exampleDraft,parseDraft,validateDraft,validateValue,scoreMetric,calculate,thresholds,formatScore,ragLabel,formatValue,RULE_VERSION,legacyWeeklyNames,isBlank,isScored,scoredCount,releaseValues,emptyUatDiscovery} from './metrics.js';
import {escapeHTML as h,badge,statsHTML,reportBody,reportDocument,csvDocument,methodologyLink,dateLabel,classificationLabels,reportFilterOptions,normalizeReportFilters,reportFilterDetails,metricBadge,discoveryNoteHTML} from './report.js';
...
function metricRows(metrics,values,scope){return metrics.map(m=>{
  ...
  const context=m.id==='uatDiscovery'?'<ul class="metric-description"><li>Use distinct confirmed defects from the same release and agreed scope.</li><li>Pre-UAT includes functional, integration, and regression discoveries.</li><li>Exclude duplicates, cancelled non-defects, and out-of-scope requirement changes.</li><li>All three counts are needed for a score; enter 0 P0/P1 UAT discoveries explicitly when none were found.</li><li>Any P0/P1 UAT discovery makes this metric Red, even if it has since been fixed.</li><li>P0/P1 UAT discoveries are part of the total UAT discoveries.</li><li>A UAT finding is not automatically a QA failure.</li></ul>':'';
...
const actions={
  ...
  'reset-release-component':async(button)=>{
    ...
    if(!confirmed)return;
    for(const m of metrics)if(!m.derived)draft.release[m.id]=m.unit==='uatShare'?emptyUatDiscovery():'';
    ...
    document.getElementById(`input-release-${first.id}${first.unit==='uatShare'?'-preUat':''}`)?.focus();
