import {weeklyMetrics,releaseMetrics,trendMetrics,blankDraft,emptyWeek,exampleDraft,parseDraft,validateDraft,validateValue,scoreMetric,calculate,thresholds,formatScore,ragLabel,formatValue,RULE_VERSION,legacyWeeklyNames,isBlank} from './metrics.js';
import {escapeHTML as h,badge,statsHTML,reportBody,reportDocument,csvDocument,methodologyLink,dateLabel,classificationLabels,reportFilterOptions,normalizeReportFilters,reportFilterDetails} from './report.js';
...
function draftSummary(){const c=calculate(draft);return `<div><span class="eyebrow">CURRENT DRAFT</span><h3>${h(draft.project.name||'Your next quality report')}</h3></div><div><b>${draft.weeks.length}</b><span>Weekly records</span></div><div><b>${c.release.count} / ${releaseMetrics.length}</b><span>Release Metrics entered</span></div><div><b>${formatScore(c.overall.score)}</b><span>Overall Quality Health</span></div>`;}
function inputHTML(m,value,key,part='') {
  const id=`input-${key}${part?'-'+part:''}`;
  const name=part?`${m.name} — ${part==='regression'?'Regression':'UAT'}`:m.name;
  const max=m.unit==='%'&&m.id!=='fixed'?100:1e9;
  return `<div class="number-input"><input type="number" id="${id}" aria-label="${h(name)}" data-metric="${h(m.id)}" ${part?`data-part="${part}"`:''} min="0" max="${max}" step="${m.unit==='count'||part?'1':'any'}" placeholder="N/A" value="${h(value??'')}" inputmode="decimal"><span>${part?(part==='regression'?'Regression':'UAT'):m.unit==='%'?'%':m.unit==='days'?'days':m.unit==='count'?'count':''}</span></div>`;
}
function metricRows(metrics,values,scope){return metrics.map(m=>{const key=scope+'-'+m.id;const invalid=validateValue(m,values[m.id]);return `<tr data-row="${m.id}"><td><label for="input-${key}${m.unit==='comparison'?'-regression':''}">${h(m.name)}</label>${classificationLabels(m)}<small>${h(m.formula)}</small><span class="source-tag">${h(m.source)}</span><details class="threshold-detail"><summary>RAG thresholds</summary><div>${thresholds(m).map((t,i)=>`<span><i class="threshold-dot ${['green','amber','red'][i]}"></i>${h(t)} · ${[100,60,0][i]} pts</span>`).join('')}</div></details></td><td class="live-input-cell">${m.unit==='comparison'?`<div class="comparison-inputs">${inputHTML(m,values[m.id]?.regression,key,'regression')}${inputHTML(m,values[m.id]?.uat,key,'uat')}</div>`:inputHTML(m,values[m.id],key)}<span class="input-error" aria-live="polite">${invalid?h(invalid):''}</span></td><td class="live-score">${scoreMetric(m,values[m.id])??'—'}</td><td class="live-rag">${invalid?'<span class="badge red">Invalid</span>':badge(scoreMetric(m,values[m.id]))}</td></tr>`;}).join('');}
...
function scoreStrip(scope){const s=scope==='release'?calculate(draft).release:calculate(draft).weeks[activeWeek]??{score:null,count:0};return `<div class="live-strip"><span><strong>${scope==='release'?'Release health':'This week’s health'}</strong><small>Blank = N/A · measured zero counts</small></span><span class="live-summary"><b>${formatScore(s.score)}</b><small>/ 100</small>${badge(s.score)}</span><span class="live-completion">${s.count} / ${scope==='release'?releaseMetrics.length:weeklyMetrics.length} measured</span></div>`;}
function releaseView(){return `<div id="live-strip">${scoreStrip('release')}</div><div class="entry-hint">Enter percentages directly, e.g. <b>95.5</b> for 95.5%. Leave unavailable measurements blank. ${methodologyLink('How scoring works','text-link')}</div>${metricGroups(releaseMetrics,draft.release,'release')}<div class="flow-bottom"><button class="button" data-action="go-weekly">← Weekly Metrics</button><button class="button primary" data-action="generate">Generate measurement report ↗</button></div>`;}
...
function refreshLive(){document.querySelectorAll('[data-row]').forEach(row=>{const m=(page==='release'?releaseMetrics:weeklyMetrics).find(m=>m.id===row.dataset.row);const value=(page==='release'?draft.release:draft.weeks[activeWeek].values)[m.id];const error=validateValue(m,value);row.querySelector('.input-error').textContent=error||'';row.querySelector('.live-score').textContent=scoreMetric(m,value)??'—';row.querySelector('.live-rag').innerHTML=error?'<span class="badge red">Invalid</span>':badge(scoreMetric(m,value));row.querySelectorAll('input').forEach(input=>input.setAttribute('aria-invalid',String(!!error)));});const strip=document.querySelector('#live-strip');if(strip)strip.innerHTML=scoreStrip(page);}
document.addEventListener('input',e=>{
  ...
  else if(el.dataset.metric){const values=page==='release'?draft.release:draft.weeks[activeWeek].values;if(el.dataset.part){values[el.dataset.metric]??={regression:'',uat:''};values[el.dataset.metric][el.dataset.part]=el.value;}else values[el.dataset.metric]=el.value;refreshLive();}
...
const actions={
  ...
  'reset-release-component':async(button)=>{
    ...
    const confirmed=await confirmReplace(`Clear all measurements in ${group}? Other release components, project details, and weekly measurements will be kept.`,{title:`Reset ${group}?`,confirmLabel:'Reset component'});
    if(!confirmed)return;
    for(const m of metrics)draft.release[m.id]=m.unit==='comparison'?{regression:'',uat:''}:'';
    errors=[];
    clearTimeout(saveTimer);
    save();
    render();
    const first=metrics[0];
    document.getElementById(`input-release-${first.id}${first.unit==='comparison'?'-regression':''}`)?.focus();
