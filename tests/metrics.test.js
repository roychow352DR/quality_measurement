...
test('Health uses actual measured denominator and original precision',()=>{const d=blankDraft();assert.equal(calculate(d).overall.score,null);d.release.escaped=2;d.release.leakage=7;d.weeks=[{start:'2026-06-01',end:'2026-06-05',values:{open:6},trends:{totalCreated:999}}];const s=calculate(d);assert.deepEqual(s.overall,{count:3,points:160,score:160/3});assert.equal(s.release.score,80);assert.equal(s.weekly.score,0);assert.equal(ragLabel(84.999),'Amber');assert.equal(ragLabel(85),'Green');assert.equal(ragLabel(59.999),'Red');});
...
test('Backup round trip, rejection and original example rescore',()=>{const d=exampleDraft();assert.deepEqual(parseDraft(JSON.parse(JSON.stringify(d))),d);assert.equal(validateDraft(d,{requireReady:true}).length,0);assert.equal(calculate(d).release.count,13);assert.equal(calculate(d).release.points,1020);assert.equal(calculate(d).release.score,1020/13);assert.equal(calculate(d).weekly.count,36);assert.throws(()=>parseDraft({}));assert.throws(()=>parseDraft({...d,weeks:new Array(105).fill(d.weeks[0])}));const bad=structuredClone(d);bad.release.leakage='abc';assert.throws(()=>parseDraft(bad));});
...
test('All metrics have exactly one agreed primary perspective',()=>{
  assert.deepEqual(weekly.filter(m=>m.perspective==='qa').map(m=>m.id),['functional','regression','firstRun','flaky']);
  assert.deepEqual(release.filter(m=>m.perspective==='qa').map(m=>m.id),['leakage','functional','completion','regression','coverage','firstRun','flaky','uatDiscovery']);
  ...
  for(const m of [...weekly,...release,...trendMetrics]){assert.ok(['qa','development'].includes(m.perspective));}
  assert.ok(trendMetrics.every(m=>m.perspective==='development'));
});
test('Perspective scores partition scored observations without counting metrics twice',()=>{
  const d=blankDraft();d.release.leakage=0;d.release.escaped=3;d.release.hotfixes=0;
  ...
  assert.deepEqual(dev.overall,{count:5,points:260,score:52});
  ...
  assert.equal(s.overall.score,520/9);assert.notEqual(s.overall.score,(65+52)/2);
  assert.equal(qa.weeks[0].score,100);assert.equal(dev.weeks[0].score,0);
  assert.equal(qa.release.score,100);assert.equal(dev.release.score,80);
  assert.equal(s.weekly.score,(50+160/4)/2);
});
test('Perspective blanks, invalid entries, partial comparisons and historical values do not score',()=>{
  ...
  s=calculate(d);assert.deepEqual(s.perspectives.qa.overall,{count:1,points:0,score:0});
  d.release.regression=90;s=calculate(d);
  assert.equal(s.perspectives.qa.overall.score,30);
