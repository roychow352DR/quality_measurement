...
test('Backup round trip, rejection and original example rescore',()=>{const d=exampleDraft();assert.deepEqual(parseDraft(JSON.parse(JSON.stringify(d))),d);assert.equal(validateDraft(d,{requireReady:true}).length,0);assert.equal(calculate(d).release.count,14);assert.equal(calculate(d).release.points,1020);assert.equal(calculate(d).release.score,1020/14);assert.equal(calculate(d).weekly.count,36);assert.throws(()=>parseDraft({}));assert.throws(()=>parseDraft({...d,weeks:new Array(105).fill(d.weeks[0])}));const bad=structuredClone(d);bad.release.leakage='abc';assert.throws(()=>parseDraft(bad));});
